...
\begin{frame}{Virtualization, the technical trick}
    A single physical server in a data center is a powerful machine, often with dozens of cores and hundreds of gigabytes of RAM, far more than most small customers need.
...
\begin{frame}{What is still missing}
    The container is now reachable at \texttt{http://<vm-ip>:8000}, which is technically a working public AI service but almost no one would actually use it.
...
\begin{frame}{Deploy your server to the cloud}
    ...
    \begin{block}{Outline}
        \begin{enumerate}
            ...
            \item Create a small VM and set up the basic runtime
...
\begin{frame}{Extensions}
    \begin{enumerate}
        ...
        \item Get an HTTPS certificate with Certbot and Let's Encrypt
...
\begin{frame}{What you should walk away with}
    ...
    \begin{enumerate}
        \item Understand what the cloud is, and why a plain VM is a sensible starting point
        \item Deploy a containerized AI server onto a cloud VM
        \item Make a service public with a domain name and HTTPS
    \end{enumerate}

    \vspace{0.5em}
    Our AI server is now a public service backed by infrastructure we do not own.
    In the next module, we look at the opposite choice: running on hardware we do own, at the edge.
\end{frame}
